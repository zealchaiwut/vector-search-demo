"""
TDD tests for issue #146: Fix eval script default port mismatch.
...
def _load_module(path, name):
    """Load a Python source file as a module without executing main()."""
...
def test_ac1_run_eval_default_port_not_7070():
    ...
    assert "7070" not in mod.SEARCH_URL, (
        "run_eval.py SEARCH_URL defaults to port 7070 "
        f"— must align with .env PORT=8000. Got: {mod.SEARCH_URL}"
    )


def test_ac1_run_eval_default_port_is_8000():
    """run_eval.py must default to port 8000 when SEARCH_URL env unset."""
    env_backup = os.environ.pop("SEARCH_URL", None)
    try:
        spec = importlib.util.spec_from_file_location(
            "run_eval_146b", EVAL_SCRIPT
        )
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        assert "8000" in mod.SEARCH_URL, (
            "run_eval.py SEARCH_URL must default to port 8000. "
...
def test_ac1_run_eval_env_override_still_works():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "run_eval_146c", EVAL_SCRIPT
        )
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        assert mod.SEARCH_URL == custom, (
            f"SEARCH_URL env override broken. "
            f"Expected {custom!r}, got {mod.SEARCH_URL!r}"
        )
    finally:
...
def test_ac2_ablation_default_url_not_7070():
    ...
    assert "7070" not in mod._DEFAULT_SEARCH_URL, (
        "run_ablation.py _DEFAULT_SEARCH_URL contains port 7070 "
        "— must align with .env PORT=8000. "
...
def test_ac2_ablation_default_url_is_8000():
    ...
    assert "8000" in mod._DEFAULT_SEARCH_URL, (
        "run_ablation.py _DEFAULT_SEARCH_URL must default to port 8000. "
        f"Got: {mod._DEFAULT_SEARCH_URL}"
    )
...
def test_ac3_readme_no_7070_in_example_comment():
    """README.md must not reference port 7070 in the eval example."""
    with open(README_PATH, encoding="utf-8") as f:
        content = f.read()
    assert "7070" not in content, (
        "src/eval/README.md still references port 7070 "
        "— must be updated to 8000."
    )
